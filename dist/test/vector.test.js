...
it('creates a valid vector', () => {
    const zeros = new src_1.Vector(3);
    // expect(zeros).toHaveLength(3);
    expect(zeros.toArr()).toEqual([0, 0, 0]);
});
test('Vector creation', () => {
    let v = new src_1.Vector([1, -3, 2]);
    expect(v.dim()).toEqual(3);
    let w = new src_1.Vector(3);
    expect(w.dim()).toEqual(3);
});
test('Vector addition', () => {
    let u = new src_1.Vector([1, -3, 2]);
    let v = new src_1.Vector([4, -5, 6]);
